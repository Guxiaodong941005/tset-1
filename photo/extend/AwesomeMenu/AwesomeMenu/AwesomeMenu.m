...
- (void)setExpanding:(BOOL)expanding
{
    ...
    [self setDelegateMothodsWithExpanding:expanding];
}
...
- (void)_expand
{
    ...
    rotateAnimation.duration = kAwesomeMenuDefaultExpandRotationDuration;
    rotateAnimation.keyTimes = [NSArray arrayWithObjects:
                                [NSNumber numberWithFloat:.3], 
                                [NSNumber numberWithFloat:.4], nil]; 
    
    CAKeyframeAnimation *positionAnimation = [CAKeyframeAnimation animationWithKeyPath:@"position"];
    positionAnimation.duration = kAwesomeMenuDefaultExpandRotationDuration;
    ...
    animationgroup.duration = kAwesomeMenuDefaultExpandRotationDuration;
...
- (void)_close
{
    ...
    rotateAnimation.duration = kAwesomeMenuDefaultCloseRotationDuration;
    rotateAnimation.keyTimes = [NSArray arrayWithObjects:
    ...
    CAKeyframeAnimation *positionAnimation = [CAKeyframeAnimation animationWithKeyPath:@"position"];
    positionAnimation.duration = kAwesomeMenuDefaultCloseRotationDuration;
    CGMutablePathRef path = CGPathCreateMutable();
    ...
    CAAnimationGroup *animationgroup = [CAAnimationGroup animation];
    animationgroup.animations = [NSArray arrayWithObjects:positionAnimation, rotateAnimation, nil];
    animationgroup.duration = kAwesomeMenuDefaultCloseRotationDuration;
    animationgroup.fillMode = kCAFillModeForwards;
    animationgroup.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseIn];
...
#pragma mark - extends mothods
-(void)setDelegateMothodsWithExpanding:(BOOL )expanding{
    SEL selector = expanding ? @selector(awesomeMenuDidFinishAnimationOpen:) : @selector(awesomeMenuDidFinishAnimationClose:);
    
    if ([_delegate respondsToSelector:selector]) {
        if (expanding) {
            [_delegate awesomeMenuDidFinishAnimationOpen:self];
        }else{
            [_delegate awesomeMenuDidFinishAnimationClose:self];
        }
    }
}
